...
import numpy as np
from xdg_parameters import par
...
class MultiStimulus:
    ...
    def __init__(self):
        ...
        # self.match_delay = np.array([200, 400])//par['dt']
    ...
    def get_tasks(self):
        if par['task'] == 'multistim':
            self.task_types = [
                [self.task_go, 'go', 0], # 0
                [self.task_go, 'rt_go', 0], # 1
                [self.task_go, 'dly_go', 0], # 2

                [self.task_go, 'go', np.pi], # 3
                [self.task_go, 'rt_go', np.pi], # 4
                [self.task_go, 'dly_go', np.pi], # 5

                [self.task_dm, 'dm1'], # 6
                [self.task_dm, 'dm2'], # 7
                [self.task_dm, 'ctx_dm1'], # 8
                [self.task_dm, 'ctx_dm2'], # 9
                [self.task_dm, 'multsen_dm'], # 10

                [self.task_dm_dly, 'dm1_dly'], # 11
                [self.task_dm_dly, 'dm2_dly'], # 12
                [self.task_dm_dly, 'ctx_dm1_dly'], # 13
                [self.task_dm_dly, 'ctx_dm2_dly'], # 14
                [self.task_dm_dly, 'multsen_dm_dly'], # 15

                [self.task_matching, 'dms'], # 16
                [self.task_matching, 'dmc'], # 17
                [self.task_matching, 'dnms'], # 18
                [self.task_matching, 'dnmc'] # 19
            ]
        elif par['task'] == 'go':
            self.task_types = [
                [self.task_go, 'go', 0], # 0
                ...
                [self.task_go, 'go', 0.5*np.pi], # 6
                [self.task_go, 'rt_go', 0.5*np.pi], # 7
                [self.task_go, 'dly_go', 0.5*np.pi], # 8

                [self.task_go, 'go', -0.5*np.pi], # 9
                [self.task_go, 'rt_go', -0.5*np.pi], # 10
                [self.task_go, 'dly_go', -0.5*np.pi] # 11
            ]
    ...
    def generate_trial(self, current_task):
        ...
        return task[1], self.trial_info['neural_input'].astype(np.float32), self.trial_info['desired_output'].astype(np.float32), \
            self.trial_info['train_mask'].astype(np.float32), self.trial_info['reward_data'].astype(np.float32)
