...
class bioRNN_Cell(tf.keras.layers.AbstractRNNCell):
    def __init__(self, n_hidden, EI=True, excitatory_frac=0.8,
        ...
        if self.structure_mask:
            assert self.structure_mask.shape == (n_hidden, n_hidden)
        self.EI = EI
        ...
        # EI stuff
        self.num_exc_units = round(excitatory_frac*n_hidden)
    ...
    def _get_synapse_config_list(self, synapse_config):
        _dict = {
            ...
            'static': ['static' for i in range(self.n_hidden)]
    ...
    def build(self, input_shape):
        ...
        """
        _w_in = initializers.gamma([input_shape[-1], self.n_hidden],
            self.input_connection_prob, shape_param=0.2)

        """
        _w_out = initializers.gamma([self.n_hidden, self.n_output], self.connection_prob)
        inh_mask = np.ones_like(_w_out)
        inh_mask[-self.num_inh_units:, :] = 0.
        _w_out *= inh_mask
        """

        _w_rnn = initializers.gamma([self.n_hidden, self.n_hidden], self.connection_prob)
        if self.balance_EI:
            _w_rnn[:, -self.num_inh_units:] = \
                initializers.gamma([self.n_hidden, self.num_inh_units], self.connection_prob,
                    shape_param=0.2)
            _w_rnn[-self.num_inh_units:, :] = \
                initializers.gamma([self.num_inh_units, self.n_hidden], self.connection_prob,
                    shape_param=0.2)
        if self.structure_mask:
            _w_rnn *= self.structure_mask
        ...
        if self.synapse_config is 'static':
            self.W_rnn.assign(self.W_rnn/3)
    ...
    def call(self, input, state):
        ...
        """
        h, syn_x, syn_u = state
        if self.synapse_config is not 'static':
            ...
            h_post = h
        old_part = (1-self.alpha_neuron)*h
        new_part = self.alpha_neuron*(input @ tf.nn.relu(self.W_in)
            + h_post @ self.EI_matrix @ tf.nn.relu(self.W_rnn) + self.b_rnn)
        noise = tf.random.normal(tf.shape(h), mean=0., stddev=self.noise_rnn)
        h = tf.nn.relu(old_part + new_part + noise)
